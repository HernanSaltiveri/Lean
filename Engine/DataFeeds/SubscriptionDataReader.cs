                ...
                try
                {
                    ...
                    if (!_config.IsCustomData && _config.SecurityType != SecurityType.Option)
                    {
                        ...
                        if (_hasScaleFactors)
                        {
                            ...
                            {
                                Log.Error($"_periodStart {_periodStart:yyy-MM-dd} | factor file first row {_factorFile.FactorFileMinimumDate.Value.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)}");
